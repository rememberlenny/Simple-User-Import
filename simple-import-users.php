	...
	if (isset($_POST['info_update'])) {
		...
		foreach ($u_temp as $ut) {
			...
			if (trim($ut) != '') {
				...
				if (! (list($u_e, $u_f, $u_l)  = @split($delimiter, $ut, 6))){
		...
		foreach ($u_data as $ud) {
			...
			$user_line = '<b>' . htmlspecialchars($ud['email']) . '|' . htmlspecialchars($ud['firstname']) . '|' . htmlspecialchars($ud['lastname']) . '</b>';
